 ...
 */
public final class SearchRequestExecutionStage implements Stage<AttributeDef> {
    @Override
    public InternalResponse<AttributeDef> execute(InternalRequest<AttributeDef> request) {
        SearchResult searchResult = request.attributes(new AttributesGrouper()).entrySet().stream()
                .map(e -> e.getKey().search(request.principal(), request.values(e.getValue())))
                .reduce(SearchResult.emptyResult(), SearchResult::merge);

        return new InternalResponse<AttributeDef>(
